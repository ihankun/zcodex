...
const settingsFile = join(homedir(), ".zcodex", "v2", "setting.json");
...
app.whenReady().then(async () => {
  ...
  // 本地定制（ZCodex fork）：禁用启动时的远端强制升级 gate（maybeBlockStartupForForceUpdate）。
  // 原因：fork 版本号独立演进（1.0.0），必然低于上游线上 release 下发的 minimalVersion，
  // gate 会永久拦截启动弹"需要升级 ZCode"。autoUpdater 常规升级检查不受影响。
  logger.info("[force-update] ZCodex fork 已禁用远端强制升级检查");
