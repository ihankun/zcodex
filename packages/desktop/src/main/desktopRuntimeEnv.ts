...
// e2e 运行的是生产构建，默认会和本机正式版 ZCodex 共用 app name / userData，
...
export const runtimeApplicationName =
  ...
  (isLocalDevelopmentRuntime ? "ZCodex Dev" : isPreviewPackagedRuntime ? "ZCodex Preview" : "ZCodex");
      ...
      : packagedDesktop
        ...
        : // Truthy grammar must match the producer's isUnsignedHelperLocalDevRequested
          ...
          ? rawInheritedEnv.ZCODE_CUA_BUNDLED_HELPER_APP_PATH?.trim() ||
            join(
              rawInheritedEnv.ZCODE_HOME?.trim() || join(homedir(), ".zcodex"),
              "computer-use",
              "dev",
              DEV_HELPER_APP_NAME,
            )
