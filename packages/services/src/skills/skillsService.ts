...
const SKILL_SETTINGS_DIR = join(resolveUserHomeDir(), ".zcodex", "v2");
const SKILL_CLI_SETTINGS_DIR = join(resolveUserHomeDir(), ".zcodex", "cli");
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.zcodex/skills/<name>` 下的目录项本身。
