...
function collectWindowsForbiddenAppInstallDirs(
  ...
  const candidates = [
    ...
    programFiles ? win32.join(programFiles, "ZCodex") : null,
    programFilesX86 ? win32.join(programFilesX86, "ZCodex") : null,
    programW6432 ? win32.join(programW6432, "ZCodex") : null,
    localAppData ? win32.join(localAppData, "Programs", "ZCodex") : null,
  ];
...
/** ~/.zcodex/v2/sessions/{workspaceHash} */
...
/** ~/.zcodex/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.zcodex/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .zcodex/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".zcodex", "v2");
  const newDir = join(newBaseDir, ".zcodex", "v2");
