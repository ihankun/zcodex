...
// 作用域两档：项目档落 `<cwd>/.zcode/workflows/`、全局档落 agent 机器的 `~/.zcodex/workflows/`。作用域由文件所在目录推得，frontmatter 不存 scope。
...
export const zcodeWorkflowsListParamsSchema = z
  .object({
    workspace: zcodeWorkspaceRefSchema,
    // 缺省即 `project`（本项目档）。给 `global` 时改扫本机 `~/.zcodex/workflows/`；此时 `workspace`
